...
!      lgnum(:) = MINLOC(lebedev_grid(:)%n,MASK=(lebedev_grid(:)%n >= n))
      ...
      ELSE
         print *, ("*** ERROR in FUNCTION get_number_of_lebedev_grid ***")
      END IF
...
!   *** Check argument values ***
      ...
      IF ((lgnum < 1) .OR. (lgnum > nlg)) THEN
         print *, ("Argument value #2 (lgnum) is out of range")
      END IF

      IF ((np < 0) .OR. (np > max_np)) THEN
         print *, ("Argument value #3 (np) is out of range")
      END IF
      ...
      CASE ("D")
         IF (MODULO(np, 3) == 0) THEN
            ...
            END DO
         ELSE
            print *, ("Subsytem D: np is not modulo 3 (check argument #3)")
         END IF
         nlgp = nlgp+nd*np/3
      CASE DEFAULT
